...
class SimpleClient(object):
    ...
    def disconnect(self):
        ...
        if not self.is_connected():
            logger.debug('[CONNECTION] Already disconnected from Mopidy Server')
            return
    ...
    def _ws_connect(self):
        ...
        self.wsa = websocket.WebSocketApp(
            url=self.ws_url,
            on_message=lambda ws, msg: self._server_response(ws, msg),
            on_error=lambda ws, ex: self._ws_error(ws, ex),
            on_open=lambda ws: self._ws_open(ws),
            on_close=lambda ws: self._ws_close(ws))
    ...
    def _ws_close(self, *args, **kwargs):
        if self.is_connected():
            logger.debug('[CONNECTION] Mopidy Server is disconnected')
    ...
    def _server_request(self, method, **kwargs):
        ...
        logger.debug("Sending: " + request.json_message)
        try:
